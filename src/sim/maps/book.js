...
import { emitParticle, doFlash, spawnBurst } from '../fx.js';
...
theme('Goo Swamp', { bg: '#1e2b1e', muddy: true , cover: 'tree' }, [
  ...
  // not a raw particle event, so remote players see the lift columns fire too
...
theme('Storm Peaks', { bg: '#1a2030' , cover: 'pillar' }, [
  ...
  { n: 'Downpour', b(m) { addStatic(m, W / 2, 630, W - 180, 40, { color: '#2a3242' }); addLava(m); }, u(m, now) { updateCrateRain(m, now, 20, 3400); for (let i = 0; i < 2; i++) emitParticle({ kind: 'spark', x: rand(0, W), y: rand(0, H - 120), vx: 0, vy: 12, life: 10, maxLife: 10, color: '#6a86b8', r: 1.5 }); } },
...
theme('The Void', { bg: '#0d0a14', stars: true , cover: 'pillar' }, [
  ...
  { n: 'The Maw', b(m) { addStatic(m, 200, 480, 320, 30, { color: '#1f1830' }); addStatic(m, W - 200, 480, 320, 30, { color: '#1f1830' }); addStatic(m, W / 2, 300, 240, 26, { color: '#1f1830' }); }, u(m, now) { for (const b of allBodies()) { if (b.isStatic || b.isSensor) continue; const dx = W / 2 - b.position.x, dy = 720 - b.position.y; const d = Math.hypot(dx, dy) || 1; if (d < 480) addVelocity(b, { x: (dx / d) * perSecond(0.3), y: (dy / d) * perSecond(0.3) }); } if (simRandom() < 0.4) emitParticle({ kind: 'square', x: W / 2 + rand(-160, 160), y: H - rand(10, 60), vx: 0, vy: 2, life: 18, maxLife: 18, color: '#a55eea', r: 2.5 }); }, s: [{ x: 200, y: 120 }, { x: W - 200, y: 120 }, { x: 340, y: 120 }, { x: W - 340, y: 120 }] },
