...
import { emitParticle, spawnParticles, spawnBurst, addShake } from '../fx.js';
...
export function updateIcicles(m, now) {
  for (const ic of m.data.icicles || []) {
    ...
    if (!ic.shakeAt) {
      ...
      emitParticle({ kind: 'square', x: ix + rand(-8, 8), y: ic.body.position.y + 20, vx: 0, vy: 1, life: 20, maxLife: 20, color: '#bfe8ff', r: 2 });
    }
  }
}
